/*
 * Fire Menu  Managers/URLBlocker.cs
...
namespace Fire.Patches.Safety
{
    public class URLBlocker
    {
        ...
        private static async void LoadBanList()
        {
            while (true)
            {
                try
                {
                    ...
                    {
                        string json = await client.GetStringAsync("https://menu.Fire.software/banned_urls");
        ...
        private static void Notify(string url, string reason)
        {
            ...
            if (shouldLog)
                LogManager.Log($"HEY!! Fire Menu blocked a potentionally DANGEROUS REQUEST to: {url} | Reason: {reason} | Assumed Assembly: {assemblyName} | Assumed File: {fileName}");
        }
        ...
        private class Patch_HttpClient
        {
            static bool Prefix(HttpRequestMessage request, ref Task<HttpResponseMessage> __result)
            {
                    ...
                    IsBanned(request.RequestUri.ToString(), out var reason))
                {
                    ...
                    {
                        Content = new StringContent("This request has been blocked by Fire Menu, as it has been marked as a unsafe site.")
                    };
