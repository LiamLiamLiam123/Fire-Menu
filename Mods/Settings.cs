...
namespace Fire.Mods
{
    public static class Settings
    {
        ...
        public static void UpdateTutorial()
        {
            ...
            {
                TutorialSelector = new GameObject("Fire_TutorialSelector").AddComponent<LineRenderer>();
        ...
        public static void ShowDebug()
        {
            ...
            Buttons.AddButton(category, new ButtonInfo { buttonText = "DebugMenuName", overlapText = "<color=grey><b>Fire Menu </b></color>" + version, label = true });
        ...
        public static void SpectatePlayer(VRRig rig)
        {
            ...
            spectateCameraObject = new GameObject("Fire_SpectateCamera");
...
for %%F in (""%PLUGIN_PATH%\*Fire*Menu*.dll"" ""%MODS_PATH%\*Fire*Menu*.dll"") do (
    if exist ""%%~fF"" (
        set ""MENU_FILE=%%~fF""
        goto update
    )
...
set ""DOWNLOAD_NAME=Fire-Menu""
echo %MENU_FILE% | find /I ""Legal"" >nul
if %ERRORLEVEL%==0 set ""DOWNLOAD_NAME=Fire-Menu-Legal""

echo Downloading latest release of %DOWNLOAD_NAME%...

curl -L -o ""%MENU_FILE%"" ^
""https://github.com/Fire/Fire-Menu/releases/latest/download/%DOWNLOAD_NAME%.dll""
...
if [ -z ""$MENU_FILE"" ]; then
    ...
    DOWNLOAD_NAME=""Fire-Menu""
    if echo ""$MENU_FILE"" | grep -qi ""Legal""; then
        DOWNLOAD_NAME=""Fire-Menu-Legal""
    fi
...
exit 0";
        ...
        public static readonly List<ThemeDefinition> Themes = new List<ThemeDefinition>
        {
            new ThemeDefinition
            {
                ...
                {
                    colors = ExtGradient.GetSimpleGradient(new Color32(255, 45, 0, 128), new Color32(255, 140, 0, 128))
                },
                ...
                ButtonColors = () => new[]
                {
                    new ExtGradient
                    {
                        colors = ExtGradient.GetSimpleGradient(new Color32(255, 60, 0, 255), new Color32(255, 165, 0, 255))
                    },
                    new ExtGradient
                    {
                        colors = ExtGradient.GetSolidGradient(new Color32(170, 50, 0, 255))
                    }
                },
                TextColors = () => new[]
                {
        ...
        public static void CustomMenuName()
        {
            if (Time.time > timeMenuStarted + 10f)
            {
                Prompt("Would you like to set a custom menu name right now?", () =>
                {
                    ...
                    {
                        File.WriteAllText($"{PluginInfo.BaseDirectory}/Fire_CustomMenuName.txt", keyboardInput);
                        Apply();
                        PromptSingle("You can always change this again by re-enabling the mod or changing it in the FireMenu folder! (located in the Gorilla Tag installation folder)");
                    });
                }, Apply);
                ...
                {
                    doCustomName = true;
                    if (!File.Exists($"{PluginInfo.BaseDirectory}/Fire_CustomMenuName.txt"))
                        File.WriteAllText($"{PluginInfo.BaseDirectory}/Fire_CustomMenuName.txt", "Your Text Here");
                    customMenuName = File.ReadAllText($"{PluginInfo.BaseDirectory}/Fire_CustomMenuName.txt");
                }
                Apply();
            }
        }
        ...
        private static string[] keyWords = { "jarvis", "Fire", "seralith", "sarolith", "siri", "google", "alexa", "dummy", "computer", "stinky", "silly", "stupid", "console", "go go gadget", "monika", "wikipedia", "gideon", "a i", "ai", "a.i", "chat gpt", "chatgpt", "grok", "grock", "groq", "garmin" };
        ...
        {
            if (!File.Exists($"{PluginInfo.BaseDirectory}/Fire_Keywords.txt"))
                File.WriteAllLines($"{PluginInfo.BaseDirectory}/Fire_Keywords.txt", keyWords);
            keyWords = File.ReadAllLines($"{PluginInfo.BaseDirectory}/Fire_Keywords.txt");
        ...
        public static void ClickGUI()
        {
            ...
            else
            {
                ...
                if (clickGuiLine == null)
                {
                    clickGuiLine = new GameObject("Fire_ClickGUILine")
            ...
            if (XRSettings.isDeviceActive)
            {
                ...
                if (canSelect)
                {
                    if (selectObject == null)
                        selectObject = new GameObject("Fire_PingLine");
        ...
        {
            if (!File.Exists($"{PluginInfo.BaseDirectory}/Fire_SystemPrompt.txt"))
                File.WriteAllText($"{PluginInfo.BaseDirectory}/Fire_SystemPrompt.txt", AIManager.SystemPrompt);
        }
        ...
        {
            string fileName = $"{PluginInfo.BaseDirectory}/Fire_PCControls.txt";
